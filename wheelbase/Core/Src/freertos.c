...
#define JOINT_STATE_PERIOD_MS   400
...
void joint_state_timer_callback(rcl_timer_t *timer, int64_t last_call_time)
{
    ...
    joint_state_msg.position.data[0] = (double)enc[0].accum_counts * COUNTS_TO_RAD; // M1
    joint_state_msg.position.data[1] = (double)enc[1].accum_counts * COUNTS_TO_RAD; // M2
    joint_state_msg.position.data[2] = (double)enc[2].accum_counts * COUNTS_TO_RAD; // M3

    joint_state_msg.velocity.data[0] = (double)enc[0].velocity_rads;
    joint_state_msg.velocity.data[1] = (double)enc[1].velocity_rads;
    joint_state_msg.velocity.data[2] = (double)enc[2].velocity_rads;
...
void StartControlTask(void const *argument)
{
    ...
    PID_Init(&pid[0], 0.01f, 0.00f, 0.0f, -0.4f, 0.4f);
    PID_Init(&pid[1], 0.01f, 0.00f, 0.0f, -0.4f, 0.4f);
    PID_Init(&pid[2], 0.01f, 0.00f, 0.0f, -0.4f, 0.4f);
    ...
    for (;;)
    {
        ...
        // Forward / backward
        if (vx >= 0.0f) {
            // Forward: [0.16, 0.16, -0.40] when vx = 0.3
            duty[0] =  0.533f * vx;
            duty[1] =  0.533f * vx;
            duty[2] = -1.333f * vx;
        } else {
            // Backward: [-0.15, -0.15, 0.225] when vx = -0.3
            duty[0] =  0.500f * vx;
            duty[1] =  0.500f * vx;
            duty[2] = -0.750f * vx;
        }
        ...
        // Left / right
        if (vy >= 0.0f) {
            // LEFT vector: [-0.20, 0.20, 0.05] when vy = +0.3
            duty[0] += -0.667f * vy;
            duty[1] +=  0.667f * vy;
            duty[2] +=  0.167f * vy;
        } else {
            // RIGHT vector: [0.20, -0.20, -0.10] when vy = -0.3
            duty[0] += -0.667f * vy;
            duty[1] +=  0.667f * vy;
            duty[2] +=  0.333f * vy;
        }
